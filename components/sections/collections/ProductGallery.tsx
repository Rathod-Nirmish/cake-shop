...
interface ProductGalleryProps {
  ...
  setSortOrder: (
    order: "Newest" | "Price: Low to High" | "Price: High to Low",
  ) => void;
  isFiltering?: boolean;
}
    ...
    <div className="flex-1 w-full">
      ...
      <div className="flex justify-between items-center mb-8 bg-muted/50 p-4 rounded-xl relative">
        <p className="text-sm font-medium">
          <span className="text-primary font-bold">{items.length}</span> Designs
          match your criteria
        </p>
          ...
          {items.length === 0 && (
            <div className="py-20 flex flex-col items-center justify-center text-center">
              ...
              <p className="text-muted-foreground max-w-sm">
                Try tweaking or clearing your filters to discover more of our
                exquisite creations.
              </p>
            </div>
          )}
          ...
          {items.length > 0 && (
            <div className="mt-16 flex flex-col items-center gap-4 break-inside-avoid">
              <p className="text-sm font-medium text-muted-foreground">
                You&apos;ve seen {displayItems.length} of {items.length}{" "}
                masterpieces
              </p>
              <div className="w-48 h-1 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(displayItems.length / items.length) * 100}%`,
                  }}
                ></div>
              </div>
